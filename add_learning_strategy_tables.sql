-- Run this once on SQL Server before enabling /api/learning-strategies in production.
-- The project already has the career table, so career is used as the source of majors.
...
BEGIN
    CREATE TABLE learning_strategy_profiles (
        ...
        CONSTRAINT fk_learning_strategy_profiles_career
...
BEGIN
    CREATE TABLE major_learning_methods (
        ...
        CONSTRAINT fk_major_learning_methods_career
            FOREIGN KEY (career_id) REFERENCES career(id),
        CONSTRAINT fk_major_learning_methods_method
            FOREIGN KEY (learning_method_id) REFERENCES learning_methods(id),
        CONSTRAINT uq_major_learning_methods
            UNIQUE (career_id, learning_method_id)
    );
END;
...
IF @it IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @it)
    ...
    VALUES (@it, N'Ngành IT cần học theo hướng thực hành, làm project đều đặn và rèn tư duy logic qua bài toán thật.',
            N'Tư duy logic|Debug|Database|Git|API|Testing',
            N'IntelliJ IDEA|VS Code|GitHub|Postman|Docker|LeetCode',
            N'Tuần 1: Ôn nền tảng lập trình và Git|Tuần 2: Học database, REST API và authentication|Tuần 3: Làm mini project fullstack|Tuần 4: Viết test, deploy thử và hoàn thiện README');

IF @eco IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @eco)
    INSERT INTO learning_strategy_profiles(career_id, description, skills, tools, weekly_roadmap)
    VALUES (@eco, N'Ngành Kinh tế nên học bằng case study, dữ liệu thực tế và luyện khả năng phân tích nguyên nhân - tác động.',
            N'Phân tích số liệu|Tư duy phản biện|Excel|Đọc báo cáo|Thuyết trình',
            N'Excel|Google Sheets|World Bank Data|Trading Economics|Notion',
            N'Tuần 1: Ôn cung cầu và thị trường|Tuần 2: Đọc case study doanh nghiệp|Tuần 3: Phân tích bộ dữ liệu nhỏ|Tuần 4: Viết báo cáo và trình bày kết luận');

IF @marketing IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @marketing)
    INSERT INTO learning_strategy_profiles(career_id, description, skills, tools, weekly_roadmap)
    VALUES (@marketing, N'Marketing cần học qua chiến dịch thật, nghiên cứu khách hàng và đo lường hiệu quả bằng dữ liệu.',
            N'Customer insight|Content|Phân tích dữ liệu|Branding|Presentation',
            N'Canva|Meta Ads Library|Google Trends|Google Analytics|Notion',
            N'Tuần 1: Học STP, 4P và persona|Tuần 2: Phân tích chiến dịch mẫu|Tuần 3: Viết content plan|Tuần 4: Tạo mini campaign và đo chỉ số');

IF @design IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @design)
    INSERT INTO learning_strategy_profiles(career_id, description, skills, tools, weekly_roadmap)
    VALUES (@design, N'Thiết kế đồ họa nên học qua portfolio, luyện mắt thẩm mỹ và nhận feedback thường xuyên.',
            N'Typography|Layout|Màu sắc|Brand identity|Visual storytelling',
            N'Figma|Adobe Illustrator|Photoshop|Behance|Pinterest',
            N'Tuần 1: Ôn bố cục, màu sắc và typography|Tuần 2: Copywork thiết kế tốt|Tuần 3: Làm bộ poster/social post|Tuần 4: Hoàn thiện portfolio mini');

IF @english IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @english)
    INSERT INTO learning_strategy_profiles(career_id, description, skills, tools, weekly_roadmap)
    VALUES (@english, N'Ngôn ngữ Anh cần học đều 4 kỹ năng, tăng input chất lượng và luyện output có phản hồi.',
            N'Listening|Speaking|Academic writing|Translation|Presentation',
            N'Anki|Oxford Learner''s Dictionaries|BBC Learning English|Grammarly|YouGlish',
            N'Tuần 1: Đánh giá trình độ và lập lịch học|Tuần 2: Shadowing và học từ vựng theo chủ đề|Tuần 3: Viết essay ngắn và sửa lỗi|Tuần 4: Thuyết trình 3 phút và tự đánh giá');

IF @medicine IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @medicine)
    INSERT INTO learning_strategy_profiles(career_id, description, skills, tools, weekly_roadmap)
    VALUES (@medicine, N'Y dược cần học chắc nền tảng, ôn lặp lại theo hệ thống và liên hệ kiến thức với tình huống lâm sàng.',
            N'Ghi nhớ hệ thống|Phân tích triệu chứng|Đọc tài liệu y khoa|Kỷ luật học tập',
            N'Anki|AMBOSS|PubMed|Visible Body|Notion',
            N'Tuần 1: Ôn giải phẫu/sinh lý nền tảng|Tuần 2: Tạo flashcard thuật ngữ và cơ chế|Tuần 3: Làm case study đơn giản|Tuần 4: Tổng ôn và sửa lỗ hổng kiến thức');

IF @engineering IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @engineering)
    INSERT INTO learning_strategy_profiles(career_id, description, skills, tools, weekly_roadmap)
    VALUES (@engineering, N'Kỹ thuật nên học bằng bài toán, mô phỏng và prototype để chuyển công thức thành năng lực giải quyết vấn đề.',
            N'Toán ứng dụng|Mô hình hóa|Đọc bản vẽ|Phân tích lỗi|Làm việc nhóm',
            N'MATLAB|AutoCAD|SolidWorks|Excel|PhET Simulations',
            N'Tuần 1: Ôn toán, vật lý và công thức nền|Tuần 2: Giải bài tập theo từng dạng|Tuần 3: Mô phỏng bài toán kỹ thuật nhỏ|Tuần 4: Làm prototype hoặc báo cáo giải pháp');

IF @law IS NOT NULL AND NOT EXISTS (SELECT 1 FROM learning_strategy_profiles WHERE career_id = @law)
    INSERT INTO learning_strategy_profiles(career_id, description, skills, tools, weekly_roadmap)
    VALUES (@law, N'Ngành Luật cần học qua văn bản pháp luật, tình huống và luyện lập luận có căn cứ.',
            N'Đọc hiểu văn bản pháp luật|Lập luận|Phản biện|Viết pháp lý|Tra cứu',
            N'Thư viện Pháp luật|Google Scholar|Notion|Zotero|Microsoft Word',
            N'Tuần 1: Ôn cấu trúc văn bản và thuật ngữ|Tuần 2: Đọc tình huống và viết brief|Tuần 3: Luyện IRAC cho bài tập|Tuần 4: Tranh luận nhóm và viết memo ngắn');
